...
def test_total_goals_markets_match_full_poisson(market, maximum, over):
    ...
    cdf = sum(math.exp(-lam) * lam**k / math.factorial(k) for k in range(maximum + 1))
...
def test_btts_is_analytic_and_complementary():
    ...
    expected = 1 - math.exp(-source.lambda_home) - math.exp(-source.lambda_away) + math.exp(-(source.lambda_home + source.lambda_away))
...
def test_max_goals_does_not_change_published_markets():
    engine = MarketProbabilityEngine()
    small = {r.market: r.p_model for r in engine.calculate_all(poisson=poisson_result(max_goals=3))}
    large = {r.market: r.p_model for r in engine.calculate_all(poisson=poisson_result(max_goals=15))}
    assert small == pytest.approx(large, abs=1e-12)


def test_semantic_hash_ignores_calculated_at():
    result = MarketProbabilityEngine().calculate(poisson=poisson_result(), market=Market.TOTAL_GOALS_OVER_2_5)
    changed = replace(result, calculated_at=datetime(2030, 1, 1, tzinfo=timezone.utc))
...
def test_unsupported_market_is_blocked():
    with pytest.raises(MarketProbabilityError) as exc:
        MarketProbabilityEngine().calculate(poisson=poisson_result(), market="CORNERS_OVER_9_5")
...
def test_success_and_blocked_logs(caplog):
    ...
    success = next(record for record in caplog.records if record.message == "market_probability_calculated")
    ...
    with caplog.at_level("WARNING"), pytest.raises(MarketProbabilityError):
        engine.calculate(poisson=replace(poisson_result(), lambda_home=0.0), market=Market.BTTS_YES)
    blocked = next(record for record in caplog.records if record.message == "market_probability_blocked")
