...
def upgrade():
    op.create_table(
        ...
        sa.Column("exposure_semantic_hash", sa.String(64), nullable=False),
        sa.Column("bankroll_amount", sa.Float(), nullable=False),
        sa.Column("unit_percent", sa.Float(), nullable=False),
        ...
        sa.UniqueConstraint(
            "semantic_hash",
            name="uq_risk_assessment_snapshots_semantic_hash",
        ),
    )
    op.create_index(
        "ix_risk_assessment_snapshots_match_asof",
        "risk_assessment_snapshots",
        ["match_id", "as_of"],
    )
...
def downgrade():
    op.drop_index(
        "ix_risk_assessment_snapshots_match_asof",
        table_name="risk_assessment_snapshots",
    )
