...
def _bounded_probability(value: float) -> float:
    if not math.isfinite(value):
        raise MarketProbabilityError(MarketProbabilityReason.INVALID_MARKET_PROBABILITY)
...
class MarketProbabilityEngine:
    def calculate_all(self, *, poisson: PoissonResult) -> tuple[MarketProbabilityResult, ...]:
        return tuple(self.calculate(poisson=poisson, market=market) for market in Market)

    def calculate(self, *, poisson: PoissonResult, market: Market | str) -> MarketProbabilityResult:
        ...
        try:
            ...
            logger.warning(
                ...
                extra={
                    ...
                    "as_of": getattr(getattr(poisson, "as_of", None), "isoformat", lambda: None)(),
                    "market": str(market),
    ...
    def _calculate(self, *, poisson: PoissonResult, market: Market | str) -> MarketProbabilityResult:
        if not isinstance(poisson, PoissonResult):
            raise MarketProbabilityError(MarketProbabilityReason.INVALID_POISSON_RESULT)
        if poisson.match_id <= 0 or poisson.as_of.tzinfo is None or poisson.as_of.utcoffset() is None:
            raise MarketProbabilityError(MarketProbabilityReason.INVALID_POISSON_RESULT)
        if poisson.model_name != POISSON_MODEL_NAME or poisson.model_version != POISSON_MODEL_VERSION:
            raise MarketProbabilityError(MarketProbabilityReason.INCOMPATIBLE_MODEL_VERSION)
        if not (_positive_finite(poisson.lambda_home) and _positive_finite(poisson.lambda_away)):
            raise MarketProbabilityError(MarketProbabilityReason.INVALID_LAMBDA)
        if not (
            math.isfinite(poisson.matrix_probability_mass)
            and math.isfinite(poisson.tail_probability)
            and -NUMERICAL_TOLERANCE <= poisson.matrix_probability_mass <= 1.0 + NUMERICAL_TOLERANCE
            and -NUMERICAL_TOLERANCE <= poisson.tail_probability <= 1.0 + NUMERICAL_TOLERANCE
            and abs(poisson.matrix_probability_mass + poisson.tail_probability - 1.0) <= NUMERICAL_TOLERANCE
        ):
            raise MarketProbabilityError(MarketProbabilityReason.INVALID_PROBABILITY_MASS)

        try:
            market_enum = market if isinstance(market, Market) else Market(market)
        except (ValueError, TypeError):
            raise MarketProbabilityError(MarketProbabilityReason.UNSUPPORTED_MARKET) from None
        ...
        if market_enum is Market.TOTAL_GOALS_OVER_1_5:
            ...
            btts_yes = _bounded_probability(1.0 - home_zero - away_zero + both_zero)
            probability = btts_yes if market_enum is Market.BTTS_YES else 1.0 - btts_yes
        ...
        return MarketProbabilityResult(
            ...
            as_of=poisson.as_of.astimezone(timezone.utc),
